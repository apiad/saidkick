...
- **2026-06-30 — bare `press Enter` flaked on focus across identical runs.** Smoke-testing the aegis web client at `127.0.0.1:8791`. After `saidkick type --css "#input"`, `saidkick press Enter --tab T` (no locator) sent the message on one run but silently no-op'd on the next with the exact same sequence — the message stayed in the textarea, no keydown reached the app's Enter handler. Pinning focus first (`saidkick click --css "#input"` then `saidkick press Enter --css "#input"`) landed it reliably. The app's handler was correct (sent cleanly once focus was pinned), so this is a saidkick focus-timing issue, not an app bug. **Proposed**: when `press` is given a locator (`--css`/`--by-*`), focus that element before dispatching the key; for the no-locator case, document that focus is best-effort and recommend pairing with a prior `click` on flaky pages.

- **2026-07-01 — corroborates 2026-06-24; server log is the real recovery signal.** Smoke-testing the AInBox dynamic-dashboards e2e (peacock artifact at `localhost:8005`). Hit the same State-3 symptom: `saidkick tabs` printed "No tabs. Is a browser connected?" while the extension was genuinely connected. I fell into the exact trap the 2026-06-24 note warns against — ran a background `saidkick start` that bound the port as a *competing* server and orphaned the extension's WS, then thrashed on Reconnect. What actually unblocked it: reading `/tmp/saidkick.log`, which showed `[status] Browser connected: br-f56c` **and** `LIST_TABS` returning empty — i.e. the browser was attached the whole time; `tabs` just returned zero enumerable tabs and the CLI collapsed that into the "no browser" message. Once I had `br-f56c` from the log I called `saidkick open --browser br-f56c <url>` directly and it worked first try — no reconnect needed. **Reinforces both 2026-06-24 proposals, plus one addition:** the empty-list `tabs` response should surface the connected browser id(s) even when zero tabs come back (e.g. "browser br-f56c connected · 0 tabs") so a script can proceed to `open` without a listing; and `saidkick doctor` should read the same server-side session registry the log reflects. Net: the connected-browser id is discoverable server-side even when `tabs` looks empty — the CLI just doesn't expose it.

- **2026-07-03 — `exec` CSS selectors mangled by the shell; CLI-outside-repo footgun.** Smoke-testing the ainbox-help docs UI + the desktop shell at `localhost:8000`. Two small frictions: (1) `saidkick exec --tab T "return !!document.querySelector('iframe[src*=8006]')"` threw `SyntaxError: 'iframe' is not a valid selector` — the `[src*=8006]` attribute selector got chewed up passing through bash into the JS string; the value reaching `querySelector` was broken. `--arg` (0.7.1+) is the sanctioned escape hatch but it's easy to forget for a quick one-liner. **Proposals:** document "use `--arg` for any selector with `[...]`/quotes in `exec`", and/or add a first-class `saidkick get-attr --tab T <locator> --attr src` (and an `exists`/`count` for locators) so ad-hoc DOM probes don't need hand-rolled `querySelector` in `exec` at all. (2) `saidkick close` / `saidkick exec` fail (`uv run` can't resolve `saidkick`) when invoked from another repo's cwd — I hit exit 2 running `close` from `repos/ainbox` mid-task. Minor, but a note in SKILL.md ("always `cd repos/saidkick` first, even for a one-off `close`") or a tiny wrapper on `$PATH` would avoid the stumble. Neither blocked the task — the docs UI + shell smoke both landed.

- **2026-07-21 — native JS dialogs (`alert`/`confirm`) block *all* saidkick commands with no way to dismiss.** E2E-testing the UNE scheduler's Pareto-frontier UI at `localhost:8091`. The app fired `window.alert()` on save-success and on transient SSE errors; while a dialog is open, every subsequent `exec`/`screenshot`/`press`/`click` on that tab times out (the renderer is dialog-modal). saidkick has no primitive to accept/dismiss a native dialog — the only recovery was `saidkick close --tab T` (which dismisses the dialog by killing the tab) and re-opening, losing page state each time. It cost several round-trips to even diagnose, since "every command times out on an active, connected tab" reads like a connection fault, not a dialog. **Proposals**, both small and CDP-backed: (1) a `saidkick dialog accept|dismiss [--text ...] --tab T` command wrapping CDP `Page.handleJavaScriptDialog` (and `Page.enable` + a listener for `Page.javascriptDialogOpening`) so a script can clear a blocking `alert`/`confirm`/`prompt` without nuking the tab; (2) an optional auto-dismiss mode (`saidkick set-dialog-policy --tab T accept|dismiss`) that installs the handler up-front so unexpected dialogs never wedge an automation run — and, relatedly, `saidkick doctor` / command-timeout errors could hint "a JS dialog may be open on this tab" when a tab is connected+active yet every command times out, to shortcut the misdiagnosis.
